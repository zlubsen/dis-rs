...
impl SerializeCdis for CdisHeader {
    fn serialize(&self, buf: &mut BitBuffer, cursor: usize) -> usize {
        let cursor = write_value_with_length::<u8>(buf, cursor, TWO_BITS, self.protocol_version.into());
        let cursor = self.exercise_id.serialize(buf, cursor);
        let cursor = write_value_with_length::<u8>(buf, cursor, EIGHT_BITS, self.pdu_type.into());
        let cursor = write_value_with_length::<u32>(buf, cursor, TWENTY_SIX_BITS, self.timestamp.raw_timestamp.into());
        let cursor = write_value_with_length(buf, cursor, FOURTEEN_BITS, self.length);
        let cursor = write_value_with_length(buf, cursor, EIGHT_BITS, dis_rs::serialize_pdu_status(&self.pdu_status, &self.pdu_type));
...
impl SerializeCdis for CdisVariableParameter {
    fn serialize(&self, buf: &mut BitBuffer, cursor: usize) -> usize {
        ...
        let cursor = write_value_with_length(buf, cursor, ONE_BIT, COMPRESSED_FLAG_TRUE);

        match self {
            CdisVariableParameter::ArticulatedPart(vp) => {
                let cursor = write_value_with_length::<u8>(buf, cursor, RECORD_TYPE_BIT_LENGTH, VariableParameterRecordType::ArticulatedPart.into());
                vp.serialize(buf, cursor)
            }
            CdisVariableParameter::AttachedPart(vp) => {
                let cursor = write_value_with_length::<u8>(buf, cursor, RECORD_TYPE_BIT_LENGTH, VariableParameterRecordType::AttachedPart.into());
                vp.serialize(buf, cursor)
            }
            CdisVariableParameter::EntitySeparation(vp) => {
                let cursor = write_value_with_length::<u8>(buf, cursor, RECORD_TYPE_BIT_LENGTH, VariableParameterRecordType::Separation.into());
                vp.serialize(buf, cursor)
            }
            CdisVariableParameter::EntityType(vp) => {
                let cursor = write_value_with_length::<u8>(buf, cursor, RECORD_TYPE_BIT_LENGTH, VariableParameterRecordType::EntityType.into());
                vp.serialize(buf, cursor)
            }
            CdisVariableParameter::EntityAssociation(vp) => {
                let cursor = write_value_with_length::<u8>(buf, cursor, RECORD_TYPE_BIT_LENGTH, VariableParameterRecordType::EntityAssociation.into());
                vp.serialize(buf, cursor)
            }
...
impl SerializeCdis for CdisArticulatedPartVP {
    fn serialize(&self, buf: &mut BitBuffer, cursor: usize) -> usize {
        let cursor = write_value_with_length(buf, cursor, EIGHT_BITS, self.change_indicator);
        let cursor = write_value_with_length(buf, cursor, TEN_BITS, self.attachment_id);
        ...
        let cursor = write_value_with_length(buf, cursor, FOURTEEN_BITS, parameter_type);
...
impl SerializeCdis for CdisAttachedPartVP {
    fn serialize(&self, buf: &mut BitBuffer, cursor: usize) -> usize {
        let cursor = write_value_with_length::<u8>(buf, cursor, ONE_BIT, self.detached_indicator.into());
        let cursor = write_value_with_length(buf, cursor, TEN_BITS, self.attachment_id);
        let cursor = write_value_with_length::<u32>(buf, cursor, ELEVEN_BITS, self.parameter_type.into());
...
impl SerializeCdis for CdisEntitySeparationVP {
    fn serialize(&self, buf: &mut BitBuffer, cursor: usize) -> usize {
        let cursor = write_value_with_length::<u8>(buf, cursor, THREE_BITS, self.reason_for_separation.into());
        let cursor = write_value_with_length::<u8>(buf, cursor, THREE_BITS, self.pre_entity_indicator.into());
        let cursor = self.parent_entity_id.serialize(buf, cursor);
        let cursor = write_value_with_length::<u16>(buf, cursor, SIX_BITS, self.station_name.into());
        let cursor = write_value_with_length::<u16>(buf, cursor, TWELVE_BITS, self.station_number);
...
impl SerializeCdis for CdisEntityTypeVP {
    fn serialize(&self, buf: &mut BitBuffer, cursor: usize) -> usize {
        let cursor = write_value_with_length::<u8>(buf, cursor, ONE_BIT, self.change_indicator.into());
        let cursor = self.attached_part_type.serialize(buf, cursor);
...
impl SerializeCdis for CdisEntityAssociationVP {
    fn serialize(&self, buf: &mut BitBuffer, cursor: usize) -> usize {
        let cursor = write_value_with_length::<u8>(buf, cursor, ONE_BIT, self.change_indicator.into());
        let cursor = write_value_with_length::<u8>(buf, cursor, FOUR_BITS, self.association_status.into());
        let cursor = write_value_with_length::<u8>(buf, cursor, EIGHT_BITS, self.association_type.into());
        let cursor = self.entity_id.serialize(buf, cursor);
        let cursor = write_value_with_length::<u16>(buf, cursor, SIX_BITS, self.own_station_location.into());
        let cursor = write_value_with_length::<u8>(buf, cursor, FIVE_BITS, self.physical_connection_type.into());
        let cursor = write_value_with_length::<u8>(buf, cursor, FOUR_BITS, self.group_member_type.into());
        let cursor = write_value_with_length::<u16>(buf, cursor, SIXTEEN_BITS, self.group_number);
