...
impl BodyProperties for Fire {
    ...
    fn body_length_bits(&self) -> usize {
        ...
        + (if let Some(_) = &self.descriptor_warhead { SIXTEEN_BITS } else { 0 })
        + (if let Some(_) = &self.descriptor_fuze { SIXTEEN_BITS } else { 0 })
        + (if let Some(_) = &self.descriptor_quantity { EIGHT_BITS } else { 0 })
        + (if let Some(_) = &self.descriptor_rate { EIGHT_BITS } else { 0 })
