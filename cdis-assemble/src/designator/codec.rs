...
impl Designator {
    fn encode(item: &Counterpart, state: Option<&EncoderStateDesignator>, options: &CodecOptions) -> (Self, CodecStateResult) {
        ...
        let (spot_wrt_designated_entity, location_wrt_entity_units) = if item.spot_wrt_designated_entity != VectorF32::default() {
        ...
        } else { (None, UnitsMeters::default()) };
        let (dr_algorithm, dr_entity_linear_acceleration) = {
            let dr_algorithm = Some(item.dead_reckoning_algorithm);
            let dr_params_entity_linear_acceleration = encode_dr_linear_acceleration(item.dead_reckoning_algorithm, &item.linear_acceleration);
        ...
        let (
            ...
            (false, UnitsDekameters::default(), None, None, None, None, None, CodecStateResult::StateUnaffected)
        } else {
            let (spot_location, world_location_altitude_units) = encode_world_coordinates(&item.spot_location);
    ...
    fn decode(&self, state: Option<&DecoderStateDesignator>, _options: &CodecOptions) -> (Counterpart, CodecStateResult) {
        ...
        let spot_wrt_designated_entity = decode_entity_coordinate_vector(&self.spot_wrt_designated_entity.unwrap_or_default(), self.units.location_wrt_entity_units);
        let dr_algorithm= self.dr_algorithm.unwrap_or_default();
        let dr_entity_linear_acceleration = self.dr_entity_linear_acceleration.unwrap_or_default().decode();
