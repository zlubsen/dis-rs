...
use crate::iff::model::{
    CdisFundamentalOperationalData, Iff, IffFundamentalParameterData, IffLayer1FieldsPresent,
...
use crate::records::parser::{
    beam_data, entity_coordinate_vector, entity_identification, layer_header,
};
...
use dis_rs::iff::model::{
    ChangeOptionsRecord, DapSource, EnhancedMode1Code, IffDataRecord, InformationLayers,
    LayersPresenceApplicability, Mode5InterrogatorStatus, Mode5MessageFormats,
    Mode5TransponderBasicData, Mode5TransponderStatus, Mode5TransponderSupplementalData,
...
pub(crate) fn iff_body(input: BitInput) -> IResult<BitInput, CdisBody> {
    ...
    let (input, event_id) = parse_field_when_present(
        fields_present,
        IffLayer1FieldsPresent::EVENT_ID_BIT,
        entity_identification,
    )(input)?;
    let (input, relative_antenna_location) = parse_field_when_present(
        fields_present,
        IffLayer1FieldsPresent::RELATIVE_ANTENNA_LOCATION_BIT,
        entity_coordinate_vector,
    )(input)?;
    let (input, system_id) = parse_field_when_present(
        fields_present,
        IffLayer1FieldsPresent::SYSTEM_ID_DETAILS_BIT,
        system_id,
    )(input)?;
    let (input, system_designator): (BitInput, u8) = take(EIGHT_BITS)(input)?;
    let (input, system_specific_data): (BitInput, Option<u8>) = parse_field_when_present(
        fields_present,
        IffLayer1FieldsPresent::SYSTEM_SPECIFIC_DATA_BIT,
        take(EIGHT_BITS),
    )(input)?;

    let (input, fundamental_operational_data) =
        fundamental_operational_data(fields_present)(input)?;

    let (input, layer_2) = if fundamental_operational_data.information_layers.layer_2
    ...
    let (input, layer_5) = if fundamental_operational_data.information_layers.layer_5
        == LayersPresenceApplicability::PresentApplicable
    {
...
fn fundamental_operational_data(
    fields_present: u16,
) -> impl Fn(BitInput) -> IResult<BitInput, CdisFundamentalOperationalData> {
    move |input: BitInput| {
        let (input, system_status): (BitInput, u8) = take(EIGHT_BITS)(input)?;
        let system_status = SystemStatus::from(system_status);

        let (input, data_field_1): (BitInput, Option<u8>) = parse_field_when_present(
            fields_present,
            IffLayer1FieldsPresent::DATA_FIELD_1_BIT,
            take(EIGHT_BITS),
        )(input)?;
        ...
        let (input, data_field_2): (BitInput, Option<u8>) = parse_field_when_present(
            fields_present,
            IffLayer1FieldsPresent::DATA_FIELD_2_BIT,
            take(EIGHT_BITS),
        )(input)?;

        let (input, parameter_1): (BitInput, Option<u16>) = parse_field_when_present(
            fields_present,
            IffLayer1FieldsPresent::PARAMETER_1_BIT,
            take(SIXTEEN_BITS),
        )(input)?;
        let (input, parameter_2): (BitInput, Option<u16>) = parse_field_when_present(
            fields_present,
            IffLayer1FieldsPresent::PARAMETER_2_BIT,
            take(SIXTEEN_BITS),
        )(input)?;
        let (input, parameter_3): (BitInput, Option<u16>) = parse_field_when_present(
            fields_present,
            IffLayer1FieldsPresent::PARAMETER_3_BIT,
            take(SIXTEEN_BITS),
        )(input)?;
        let (input, parameter_4): (BitInput, Option<u16>) = parse_field_when_present(
            fields_present,
            IffLayer1FieldsPresent::PARAMETER_4_BIT,
            take(SIXTEEN_BITS),
        )(input)?;
        let (input, parameter_5): (BitInput, Option<u16>) = parse_field_when_present(
            fields_present,
            IffLayer1FieldsPresent::PARAMETER_5_BIT,
            take(SIXTEEN_BITS),
        )(input)?;
        let (input, parameter_6): (BitInput, Option<u16>) = parse_field_when_present(
            fields_present,
            IffLayer1FieldsPresent::PARAMETER_6_BIT,
            take(SIXTEEN_BITS),
        )(input)?;
...
fn fundamental_parameter_data_record(
    input: BitInput,
) -> IResult<BitInput, IffFundamentalParameterData> {
...
) -> impl Fn(BitInput) -> IResult<BitInput, IffLayer3> + '_ {
    move |input: BitInput| {
        ...
        Ok((
            ...
            IffLayer3 {
                ...
                mode_5_basic_data: mode_5_basic_data.unwrap_or(Mode5BasicData::Transponder(
...
fn mode_5_interrogator_basic_data(
    input: BitInput,
) -> IResult<BitInput, Mode5InterrogatorBasicData> {
...
fn iff_layer_4(
    iff_system_type: &IffSystemType,
) -> impl Fn(BitInput) -> IResult<BitInput, IffLayer4> + '_ {
    move |input: BitInput| {
        let (input, data_records_present): (BitInput, u8) = take(ONE_BIT)(input)?;
        let data_records_present = data_records_present != 0;
        let (input, layer_header) = layer_header(input)?;
        ...
        Ok((
            ...
            IffLayer4 {
                ...
                reporting_simulation_site,
                reporting_simulation_application,
                mode_s_basic_data: mode_s_basic_data.unwrap_or(ModeSBasicData::Transponder(
...
fn mode_s_interrogator_basic_data(
    input: BitInput,
) -> IResult<BitInput, ModeSInterrogatorBasicData> {
    let (input, interrogator_status): (BitInput, u8) = take(EIGHT_BITS)(input)?;
