...
pub(crate) fn encode_iff_body_and_update_state(dis_body: &Counterpart,
    ...
    if state_result == CodecStateResult::StateUpdateIff {
        state.iff.entry(dis_body.emitting_entity_id)
...
pub(crate) fn decode_iff_body_and_update_state(cdis_body: &Iff,
                                               state: &mut DecoderState,
                                               options: &CodecOptions) -> (PduBody, CodecStateResult) {
    let state_for_id = state.iff.get(&dis_rs::model::EntityId::from(&cdis_body.emitting_entity_id));
    let (dis_body, state_result) = cdis_body.decode(state_for_id, options);

    if state_result == CodecStateResult::StateUpdateIff {
        state.iff.entry(dis_rs::model::EntityId::from(&cdis_body.emitting_entity_id))
            .and_modify(|iff| {
...
fn encode_iff_relative_antenna_location(vector: &VectorF32) -> (Option<EntityCoordinateVector>, UnitsMeters) {
...
fn encode_iff_fundamental_operational_data(data: &FundamentalOperationalData) -> CdisFundamentalOperationalData {
    CdisFundamentalOperationalData {
        system_status: data.system_status,
        data_field_1: if data.data_field_1 != 0 { Some(data.data_field_1) } else { None },
        information_layers: data.information_layers,
        data_field_2: if data.data_field_2 != 0 { Some(data.data_field_2) } else { None },
        parameter_1: if data.parameter_1 != 0 { Some(data.parameter_1) } else { None },
        parameter_2: if data.parameter_2 != 0 { Some(data.parameter_2) } else { None },
        parameter_3: if data.parameter_3 != 0 { Some(data.parameter_3) } else { None },
        parameter_4: if data.parameter_4 != 0 { Some(data.parameter_4) } else { None },
        parameter_5: if data.parameter_5 != 0 { Some(data.parameter_5) } else { None },
        parameter_6: if data.parameter_6 != 0 { Some(data.parameter_6) } else { None },
    }
}

fn decode_iff_fundamental_operational_data(data: &CdisFundamentalOperationalData) -> FundamentalOperationalData {
...
impl Iff {
    pub fn encode(item: &Counterpart, state: Option<&EncoderStateIff>, options: &CodecOptions) -> (Self, CodecStateResult) {
        let (system_id, state_result) = match options.update_mode {
            ...
            CodecUpdateMode::PartialUpdate => {
                if state.is_some_and(|state|
                    !evaluate_timeout_for_iff(&state.heartbeat, options)) {
        ...
        let system_specific_data= if item.system_specific_data == 0 { None } else { Some(item.system_specific_data) };
        let fundamental_operational_data = encode_iff_fundamental_operational_data(&item.fundamental_operational_data);
    ...
    pub fn decode(&self, state: Option<&DecoderStateIff>, options: &CodecOptions) -> (Counterpart, CodecStateResult) {
        let (system_id, state_result) = match options.update_mode {
            CodecUpdateMode::FullUpdate => {
                (self.system_id.unwrap_or_default(), CodecStateResult::StateUnaffected)
            }
            CodecUpdateMode::PartialUpdate => {
                if self.full_update_flag {
                    (self.system_id.unwrap_or_default(), CodecStateResult::StateUpdateIff)
                } else {
                    (self.system_id.unwrap_or_else(|| if let Some(state) = state { state.system_id } else { SystemId::default() } ), CodecStateResult::StateUnaffected)
                }
            }
        };

        let relative_antenna_location = if let Some(relative_antenna_location) = self.relative_antenna_location {
            decode_entity_coordinate_vector(&relative_antenna_location, self.relative_antenna_location_units)
        ...
        let builder = Counterpart::builder()
            ...
            .with_fundamental_operational_data(decode_iff_fundamental_operational_data(&self.fundamental_operational_data));
...
impl Codec for IffLayer2 {
    ...
    fn encode(item: &Self::Counterpart) -> Self {
        let mut layer = Self {
            ...
            iff_fundamental_parameters: item.iff_fundamental_parameters.iter().map(|param| IffFundamentalParameterData::encode(param) ).collect(),
        };
