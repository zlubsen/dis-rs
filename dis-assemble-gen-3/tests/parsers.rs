use dis_assemble_gen_3::enumerations::{Appearance, Country, DeadReckoningAlgorithm, EntityKind, ExtensionRecordTypes, ForceID, PlatformDomain};
use dis_assemble_gen_3::{ExtensionRecordBody, PduBody};

#[test]
fn test_parse_pdu_es() {
    let buffer: [u8; 72] = [
...
fn test_parse_pdu_es_with_extension_records() {
    let buffer: [u8; 88] = [
        ...
        0x40, 0x00, // ER 1: length 64 bytes
        ...
        // 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        // 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        // 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // padding to 64 bytes
    ];
    let parsed = dis_assemble_gen_3::parse(&buffer).unwrap();
    let pdu = parsed.first().unwrap();

    if let PduBody::EntityState(es) = &pdu.body {
        let first = es.extension_records.get(0).unwrap();
        ...
        if let ExtensionRecordBody::EntityAppearance(apps) = &first.body {
            ...
            } else { panic!("Appearance is not of type PlatformAir") }
        } else { panic!("ExtensionRecord is not of type EntityAppearance") }
    }
}
