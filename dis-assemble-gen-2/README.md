...
`dis-assemble-gen-2` is an implementation of the `Distributed Interactive Simulation` (`DIS`) protocol _generation 2_
for Rust.
...
The library supports both versions `6` and `7` of the standard ('Generation 2').
...
Here is an overview of the `DIS` `PDU`s/features supported by `dis-assemble-gen-2`:
...
`dis-assemble-gen-2` uses the `SISO-REF-010` reference to map the wire level encoding to actual names of enumerations
and values in
code.
