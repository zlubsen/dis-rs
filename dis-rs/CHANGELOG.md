...
- Fixed Variable Parameter `ArticulatedPart` field `change_indicator` to be of type `u8` instead of enumeration
  `ChangeIndicator`.
...
- Bitfield enumerations from SISO-REF-010 were incorrectly reversed during serialization/deserialization.
...
- Incorrect padding calculation for serialization of `VariableDatum`. Thanks to [@fpena2](https://github.com/fpena2).
- Incorrect `TIME_UNITS_PER_HOUR` calculation. Thanks to [@carlocorradini](https://github.com/carlocorradini).
...
- Split `DescriptorRecord` into `FireDescriptor` and `DetonationDescriptor` records.
