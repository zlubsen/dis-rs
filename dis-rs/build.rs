...
mod generation {
        ...
        F: Fn(usize) -> Option<&'a GenerationItem>,
    {
        fields.iter().map(|field| {
            ...
            let bitmask = Literal::usize_unsuffixed(2usize.pow(u32::try_from(field.length).expect("Cast `usize` to `u32`")) - 1);
    ...
    fn discriminant_literal(value: usize, data_size: usize) -> Literal {
        match data_size {
            ...
            32 => Literal::u32_suffixed(u32::try_from(value).expect("Cast `usize` to `u32`")),
            16 => Literal::u16_suffixed(u16::try_from(value).expect("Cast `usize` to `u16`")),
            _ => Literal::u8_suffixed(u8::try_from(value).expect("Cast `usize` to `u8`")),
        }
    }
}
