...
The code for these enums is generated using a build script from the published [SISO-REF-010.xml](./enumerations/SISO-REF-010.xml) file.

## Usage

### Constructing PDUs

`PDU`s are constructed using a `default()` or `builder()` associated functions on the structs for `PduHeader` or `PduBody`s.
