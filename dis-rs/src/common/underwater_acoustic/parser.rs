...
use crate::enumerations::{APAStatus, UAAcousticEmitterSystemFunction, UAAcousticSystemName, UAActiveEmissionParameterIndex, UAAdditionalPassiveActivityParameterIndex, UAPassiveParameterIndex, UAPropulsionPlantConfiguration, UAScanPattern, UAStateChangeUpdateIndicator};
...
pub(crate) fn underwater_acoustic_body(input: &[u8]) -> IResult<&[u8], PduBody> {
    ...
    let state_change_update_indicator = UAStateChangeUpdateIndicator::from(state_change_update_indicator);
    ...
    let (input, emitter_systems) = count(ua_emitter_system, number_of_emitter_systems as usize)(input)?;

    Ok((input, UnderwaterAcoustic::builder()
...
fn propulsion_plant_configuration(input: &[u8]) -> IResult<&[u8], PropulsionPlantConfiguration> {
    ...
    Ok((input, PropulsionPlantConfiguration::default()
...
fn apa(input: &[u8]) -> IResult<&[u8], APA> {
    ...
    let parameter =
        UAAdditionalPassiveActivityParameterIndex::from(parameter_index >> 2);
    let parameter_status =
        APAStatus::from((parameter_index & LAST_TWO_BITS_MASK) as u8);
    let (input, value) = be_i16(input)?;

    Ok((input, APA::default()
...
fn ua_emitter_system(input: &[u8]) -> IResult<&[u8], UAEmitterSystem> {
    ...
    Ok((input, UAEmitterSystem::default()
...
fn acoustic_emitter_system(input: &[u8]) -> IResult<&[u8], AcousticEmitterSystem> {
    ...
    Ok((input, AcousticEmitterSystem::default()
...
fn ua_fundamental_parameter_data(input: &[u8]) -> IResult<&[u8], UAFundamentalParameterData> {
    ...
    let active_emission_parameter_index = UAActiveEmissionParameterIndex::from(active_emission_parameter_index);
    ...
    Ok((input, UAFundamentalParameterData::default()
