...
mod tests {
    use bytes::BytesMut;
    use crate::enumerations::{CoupledExtensionIndicator, LvcIndicator, PduType, RadioAttachedIndicator, TransferredEntityIndicator, TransmitterTransmitState, VariableRecordType};
    ...
    fn transmitter_internal_consistency_v7() {
        let header = PduHeader::new_v7(1, PduType::Transmitter)
        ...
        let body = Transmitter::builder()
            ...
            .with_antenna_location(Location::new(0.0, 0.0, 0.0))
            .with_relative_antenna_location(VectorF32::default())
            .with_variable_transmitter_parameter(VariableTransmitterParameter::default().with_record_type(VariableRecordType::Age_34100).with_fields(vec![0xFF, 0xEE]))
            .build()
            .into_pdu_body();
        let original_pdu = Pdu::finalize_from_parts(header, body, DisTimeStamp::new_absolute_from_secs(100));
        let pdu_length = original_pdu.header.pdu_length;
