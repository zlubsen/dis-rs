...
pub(crate) fn aggregate_state_body(input: &[u8]) -> IResult<&[u8], PduBody> {
    ...
    let (input, silent_aggregate_systems) = count(silent_aggregate_system, number_of_silent_aggregates.into())(input)?;
    let (input, silent_entity_systems) = count(silent_entity_system, number_of_silent_entities.into())(input)?;

    let (input, number_of_variable_datums) = be_u32(input)?;
    let (input, variable_datums) = count(variable_datum, number_of_variable_datums as usize)(input)?;

    Ok((input, AggregateState::builder()
...
fn silent_aggregate_system(input: &[u8]) -> IResult<&[u8], SilentAggregateSystem> {
    ...
    Ok((input, SilentAggregateSystem::default()
...
fn silent_entity_system(input: &[u8]) -> IResult<&[u8], SilentEntitySystem> {
    ...
    let (input, appearances) = count(entity_appearance(entity_type), number_of_appearance_records.into())(input)?;

    Ok((input, SilentEntitySystem::default()
