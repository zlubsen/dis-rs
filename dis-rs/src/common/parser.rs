...
use crate::constants::{
    EIGHT_OCTETS, FIVE_LEAST_SIGNIFICANT_BITS, ONE_BYTE_IN_BITS, PDU_HEADER_LEN_BYTES,
};
...
pub(crate) fn vec3_f32(input: &[u8]) -> IResult<&[u8], VectorF32> {
    ...
    Ok((
        input,
        VectorF32 {
            first_vector_component: *elements
                .get(0)
                .expect("Value supposed to be parsed successfully"),
            second_vector_component: *elements
                .get(1)
                .expect("Value supposed to be parsed successfully"),
            third_vector_component: *elements
                .get(2)
                .expect("Value supposed to be parsed successfully"),
        },
    ))
...
pub(crate) fn variable_datum(input: &[u8]) -> IResult<&[u8], VariableDatum> {
    ...
    let padded_record = length_padded_to_num(datum_length_bytes, EIGHT_OCTETS);
...
pub(crate) fn variable_parameter(input: &[u8]) -> IResult<&[u8], VariableParameter> {
    ...
    let (input, variable_parameter) = match parameter_type {
        ...
        VariableParameterRecordType::Unspecified(_) => {
            ...
            (
                input,
                VariableParameter::Unspecified(
                    parameter_type_designator,
                    <[u8; 15]>::try_from(bytes).unwrap(),
                ),
            )
        } // TODO sensible error
    };
...
fn attached_part(input: &[u8]) -> IResult<&[u8], VariableParameter> {
    ...
    Ok((
        input,
        VariableParameter::Attached(AttachedPart {
            detached_indicator,
            attachment_id,
...
fn entity_association(input: &[u8]) -> IResult<&[u8], VariableParameter> {
    ...
    Ok((
        input,
        VariableParameter::EntityAssociation(EntityAssociationParameter {
            ...
            physical_connection_type: EntityAssociationPhysicalConnectionType::from(
...
fn entity_type_variable_parameter(input: &[u8]) -> IResult<&[u8], VariableParameter> {
    ...
    Ok((
        input,
        VariableParameter::EntityType(EntityTypeParameter {
            change_indicator: ChangeIndicator::from(change_indicator),
            entity_type,
        }),
    ))
...
fn separation(input: &[u8]) -> IResult<&[u8], VariableParameter> {
    ...
    Ok((
        input,
        VariableParameter::Separation(SeparationParameter {
...
pub(crate) fn record_specification(input: &[u8]) -> IResult<&[u8], RecordSpecification> {
    ...
    Ok((
        input,
        RecordSpecification::default().with_record_sets(record_sets),
    ))
}
...
mod tests {
    ...
    fn parse_header_unspecified_version() {
        let bytes: [u8; 12] = [
            0x1F, 0x01, 0x01, 0x01, 0x4e, 0xea, 0x3b, 0x60, 0x00, 0x0c, 0x00, 0x00,
        ];
    ...
    fn parse_header_body_too_short() {
        ...
        assert_eq!(
            error,
            DisError::InsufficientPduLength(208 - PDU_HEADER_LEN_BYTES, 2)
        );
    }
    ...
    fn parse_multiple_headers() {
        let bytes: [u8; 24] = [
            0x06, 0x01, 0x01, 0x01, 0x4e, 0xea, 0x3b, 0x60, 0x00, 0x0c, 0x00, 0x00, 0x06, 0x01,
            0x01, 0x01, 0x4e, 0xea, 0x3b, 0x60, 0x00, 0x0c, 0x00, 0x00,
        ];
    ...
    fn parse_multiple_headers_1st_too_short() {
        let bytes: [u8; 23] = [
            0x06, 0x01, 0x01, 0x01, 0x4e, 0xea, 0x3b, 0x60, 0x00, 0x0c, 0x00, 0x06, 0x01, 0x01,
            0x01, 0x4e, 0xea, 0x3b, 0x60, 0x00, 0x0c, 0x00, 0x00,
        ];
    ...
    fn parse_multiple_headers_too_short() {
        let bytes: [u8; 11] = [
            0x06, 0x01, 0x01, 0x01, 0x4e, 0xea, 0x3b, 0x60, 0x00, 0x60, 0x00,
        ];
