...
pub(crate) fn sees_body(input: &[u8]) -> IResult<&[u8], PduBody> {
    ...
    let (input, propulsion_systems) = count(propulsion_system_data, nr_of_propulsion_systems.into())(input)?;
    let (input, vectoring_nozzle_systems) = count(vectoring_nozzle_system_data, nr_of_vectoring_nozzle_systems.into())(input)?;
...
fn propulsion_system_data(input: &[u8]) -> IResult<&[u8], PropulsionSystemData> {
    ...
    Ok((input, PropulsionSystemData::default()
...
fn vectoring_nozzle_system_data(input: &[u8]) -> IResult<&[u8], VectoringNozzleSystemData> {
    ...
    Ok((input, VectoringNozzleSystemData::default()
