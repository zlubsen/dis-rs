...
pub enum DisError {
    ...
    #[error(
        "The buffer does not contain enough bytes for a valid DIS header. {0} bytes available, needed {PDU_HEADER_LEN_BYTES}"
    )]
    ...
    #[error(
        "PDU is larger than size of the buffer for serialisation. Needs {0} bytes, available {1} bytes"
    )]
