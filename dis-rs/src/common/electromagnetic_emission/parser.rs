...
pub(crate) fn emission_body(_header: &PduHeader) -> impl Fn(&[u8]) -> IResult<&[u8], PduBody> + '_ {
    move |input| {
        ...
        let body = ElectromagneticEmission::builder()
            ...
            .with_state_update_indicator(ElectromagneticEmissionStateUpdateIndicator::from(
                status_update_indicator,
            ))
...
mod tests {
    ...
    fn parse_pdu_emission_basic() {
        let bytes: [u8; 48] = [
            0x06, 0x01, 0x17, 0x06, 0xaa, 0x58, 0xbe, 0xc1, 0x00, 0x30, 0x00, 0x00, 0x01, 0xf4,
            0x2b, 0x67, 0x00, 0x27, 0x01, 0xf4, 0x2b, 0x67, 0x01, 0xd2, 0x00, 0x01, 0x00, 0x00,
            0x05, 0x00, 0x00, 0x00, 0x4d, 0x58, 0x02, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x41, 0x70, 0x00, 0x00,
        ];
        ...
        if let PduBody::ElectromagneticEmission(pdu) = pdu.body {
            ...
            assert_eq!(
                pdu.emitting_entity_id.simulation_address.application_id,
                11111u16
            );
            assert_eq!(pdu.emitting_entity_id.entity_id, 39u16);
            assert_eq!(pdu.emitter_systems.len(), 1);
            assert_eq!(
                pdu.emitter_systems.first().unwrap().name,
                EmitterName::Unnamed_19800
            );
            assert_eq!(
                pdu.emitter_systems.first().unwrap().function,
                EmitterSystemFunction::EarlyWarningSurveillance_2
            );
    ...
    fn parse_pdu_emission_with_tracks() {
        let bytes: [u8; 228] = [
            0x06, 0x01, 0x17, 0x06, 0xb4, 0x97, 0xa2, 0xe3, 0x00, 0xe4, 0x00, 0x00, 0x01, 0xf4,
        ...
        if let PduBody::ElectromagneticEmission(pdu) = pdu.body {
            ...
            assert_eq!(
                beam.beam_function,
                ElectromagneticEmissionBeamFunction::Illumination
            );
            assert_eq!(
                beam.high_density_track_jam,
