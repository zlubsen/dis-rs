...
fn extract_numeric_field(element: &BytesStart) -> NumericField {
    ...
    NumericField {
        ...
        primitive_type: attr_type.expect("Expected NumericField attribute 'type' to be present."),
        units: None,
    }
}
...
fn extract_fixed_record_field(
    ...
    FixedRecordField {
        ...
        field_type: attr_type.expect("Expected FixedRecordField attribute 'type' to be present."),
    }
}
...
fn extract_fixed_string_field(
    ...
    FixedStringField {
        ...
        length: attr_length.expect("Expected FixedStringField attribute 'length' to be present."),
    }
}
...
fn extract_variable_string_element(reader: &mut Reader<BufReader<File>>) -> VariableString {
    ...
    VariableString {
        count_field: count_field.expect("Expected VariableString to have an element CountField."),
    ...
    loop {
        match reader.read_event_into(&mut buf) {
            Ok(Event::Empty(ref element)) => match element.name() {
                ...
                ENUM_FIELD_ELEMENT => {
                    type_field = Some(ArrayFieldEnum::Enum(extract_enum_field(element, reader)));
                }
                FIXED_STRING_FIELD_ELEMENT => {
